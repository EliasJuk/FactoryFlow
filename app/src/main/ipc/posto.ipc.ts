...
export function registerPostoIpc() {
  ...
  ipcMain.handle('postos:criar', async (_, nome: string, subsetorId: number, usuarioId: number) => {
    try {
      await postoService.criar(nome, subsetorId, usuarioId)
  ...
  ipcMain.handle(
    'postos:editar',
    async (_, id: number, nome: string, subsetorId: number, usuarioId: number) => {
      try {
        await postoService.editar(id, nome, subsetorId, usuarioId)
  ...
  ipcMain.handle('postos:excluir', async (_, id: number, usuarioId: number) => {
    try {
      await postoService.excluir(id, usuarioId)
  ...
  ipcMain.handle('postos:restaurar', async (_, id: number, usuarioId: number) => {
    try {
      await postoService.restaurar(id, usuarioId)
