...
export class ExportacaoRepository {
  async gerarCsvRefugos(filtros: ExportacaoFiltros) {
    const result = await pool.query<LinhaExportacao>(`
      SELECT
      ...
      WHERE r.status = 'ATIVO'
        AND DATE(r.data_hora) >= DATE($1)
        AND DATE(r.data_hora) <= DATE($2)

      ORDER BY r.data_hora ASC, r.id ASC
    `, [filtros.dataInicio, filtros.dataFim])

    return montarCsv(result.rows)
  }
}
