  ...
  runBaseEntitiesBackfill(): void {
    ...
    db.transaction(() => {
      const usuarios = db
        ...
        .all(SYSTEM_IDS.usuarioSistema) as Array<{ id: number }>

      for (const usuario of usuarios) {
        this.queue.enqueueUsuario(usuario.id, 'CREATE', true)
      }

      const solicitacoesSenha = db
        .prepare(
          `
          SELECT id
      ...
      for (const solicitacao of solicitacoesSenha) {
        this.queue.enqueueSolicitacaoAlteracaoSenha(
          solicitacao.id,
          'CREATE',
          true
        )
      ...
      const setores = db
        .prepare('SELECT id FROM setores ORDER BY id')
        .all() as Array<{ id: number }>

      for (const setor of setores) {
        this.queue.enqueueSetor(setor.id, 'CREATE', true)
      }

      const subsetores = db
        .prepare('SELECT id FROM subsetores ORDER BY id')
        .all() as Array<{ id: number }>

      for (const subsetor of subsetores) {
        this.queue.enqueueSubsetor(subsetor.id, 'CREATE', true)
      }

      const postos = db
        .prepare('SELECT id FROM postos ORDER BY id')
        .all() as Array<{ id: number }>

      for (const posto of postos) {
        this.queue.enqueuePosto(posto.id, 'CREATE', true)
      }

      const componentes = db
        .prepare('SELECT id FROM componentes ORDER BY id')
        .all() as Array<{ id: number }>
      ...
      const circuitos = db
        .prepare('SELECT id FROM circuitos ORDER BY id')
        .all() as Array<{ id: number }>
      ...
      for (const circuitoComponente of circuitoComponentes) {
        this.queue.enqueueCircuitoComponente(
          circuitoComponente.id,
          'CREATE',
          true
        )
      }
      ...
      const defeitos = db
        .prepare('SELECT id FROM defeitos ORDER BY id')
        .all() as Array<{ id: number }>
      ...
      const postoDefeitos = db
        .prepare('SELECT id FROM posto_defeitos ORDER BY id')
        .all() as Array<{ id: number }>

      for (const postoDefeito of postoDefeitos) {
        this.queue.enqueuePostoDefeito(
          postoDefeito.id,
          'CREATE',
          true
        )
