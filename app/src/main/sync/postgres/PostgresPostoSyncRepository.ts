...
import type { PostoSyncPayload } from '../sync.types'

export class PostgresPostoSyncRepository {
  async apply(payload: PostoSyncPayload): Promise<void> {
    const client = await pool.connect()
    try {
      await client.query('BEGIN')
      const r = payload.record
      const subsetorId = await this.requiredId(client, 'subsetores', r.subsetorUuid)
  ...
  private async requiredId(client: PoolClient, table: 'subsetores', uuid: string): Promise<number> {
