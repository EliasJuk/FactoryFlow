...
export function CircuitoComponenteInfoModal({ item, onFechar }: Props) {
  return (
    <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/40 p-4">
      <div className="w-full max-w-xl rounded-lg bg-white p-5 shadow-xl">
        <div className="mb-4 flex items-start justify-between">
          <div>
            <h2 className={ui.title}>Informações da composição</h2>
        ...
        <div className="grid gap-3 text-sm">
          ...
          {!item.ativo && (
            <div className="rounded-md border border-[var(--border)] p-3">
              <strong>Removido por:</strong> {item.deletedByNome ?? 'Não registrado'}
              <br />
              <strong>Removido em:</strong> {formatarData(item.deletedAt)}
            </div>
          )}
        </div>
