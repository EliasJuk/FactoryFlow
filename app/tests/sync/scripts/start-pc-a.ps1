...
$env:FACTORYFLOW_SQLITE_PATH = Join-Path $databaseDirectory "pc-a.db"

$template = Join-Path $appRoot "tests/sync/configs/pc-a.json"
$target = Join-Path $configFileDirectory "config.json"

if (-not (Test-Path $template)) {
  throw "O arquivo de configuracao do PC-A nao foi encontrado: $template"
}
...
if ([string]::IsNullOrWhiteSpace($env:PGPASSWORD)) {
  throw "PGPASSWORD nao foi recebida pelo PC-A."
}
